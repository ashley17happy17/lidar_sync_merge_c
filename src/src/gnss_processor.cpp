...
void WGS84toTWD97(double lat, double lon, double& x, double& y) {
    ...
    double M = a * ((1 - e2/4 - 3*std::pow(e2, 2)/64 - 5*std::pow(e2, 3)/256)*lat_rad 
                - (3*e2/8 + 3*std::pow(e2, 2)/32 + 45*std::pow(e2, 3)/1024)*std::sin(2*lat_rad) 
                + (15*std::pow(e2, 2)/256 + 45*std::pow(e2, 3)/1024)*std::sin(4*lat_rad) 
                - (35*std::pow(e2, 3)/3072)*std::sin(6*lat_rad));
                
    x = dx + k0 * N * (A + (1 - T + C) * std::pow(A, 3) / 6 
            + (5 - 18*T + std::pow(T, 2) + 72*C - 58*e_p2) * std::pow(A, 5) / 120);
            
    y = k0 * (M + N * std::tan(lat_rad) * (std::pow(A, 2) / 2 
            + (5 - T + 9*C + 4*std::pow(C, 2)) * std::pow(A, 4) / 24 
            + (61 - 58*T + std::pow(T, 2) + 600*C - 330*e_p2) * std::pow(A, 6) / 720));
}

void processGNSSData(const std::string& gnss_file_path, std::vector<GNSSData>& gnss_data_list, Eigen::Vector3d& local_origin) {
  std::cout << "[INFO] Processing GNSS data with WGS84 -> TWD97-2010 projection -> Local Cartesian: " << gnss_file_path << std::endl;
  ...
  while (std::getline(gnss_file, line)) {
    ...
    try {
      ...
      double z = h; // Or h - geoidSep if orthometric height is strictly required

      // Capture the first valid global TWD97 point as the Local Origin
      if (!origin_set) {
          local_origin = Eigen::Vector3d(x, y, z);
          origin_set = true;
          std::cout << "[INFO] Set Local Cartesian Origin to TWD97: X=" << x << ", Y=" << y << ", Z=" << z << std::endl;
      }
  ...
  gnss_file.close();
  std::cout << "[INFO] Successfully loaded " << gnss_data_list.size()
            << " GNSS records.\n";
}
